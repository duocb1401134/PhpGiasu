...
class Database {
    ...
    function insert($table, $data) {
        ...
        foreach ($data as $key => $value) {
            ...
            $value_list .= ",'" . mysqli_real_escape_string($this->__connect, $value) . "'";
        }
    ...
    function update($table, $data, $where) {
        ...
        // lap lai data
        foreach ($data as $key => $value) {
            $sql .= "$key = '" . mysqli_real_escape_string($this->__connect, $value) . "',";
        }
        $sql = 'Update ' . $table . ' set ' . trim($sql, ',') . ' Where ' . $where;

        mysqli_set_charset($this->__connect, "utf8");
        return mysqli_query($this->__connect, $sql);
    }
    ...
    function last_id() {
        return mysqli_insert_id($this->__connect);
    }
